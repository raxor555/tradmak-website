...
const FadeInItem = ({ children, delay = 0, className = "" }: { children: React.ReactNode, delay?: number, className?: string }) => {
              ...
              <div className="flex h-full items-center gap-1 rounded-full px-2">
                {navItems.map((item) => (
                  <a 
                    key={item.label} 
                    href={item.href} 
                    className="text-xs font-medium uppercase tracking-widest text-gray-400 hover:text-white hover:bg-white/5 px-4 py-2 rounded-none transition-all duration-300"
                  >
                    {item.label}
                  </a>
                ))}
        ...
        <div className="relative z-10 flex flex-col h-full justify-center px-8 space-y-8">
          {navItems.map((item) => (
            <a 
              ...
              className="text-4xl font-bold text-white uppercase tracking-tighter hover:text-accent-primary transition-colors border-l-2 border-transparent hover:border-accent-primary pl-4"
            >
              {item.label}
            </a>
          ))}
