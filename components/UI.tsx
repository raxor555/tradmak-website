...
export const GridSystem: React.FC<{ className?: string }> = ({ className = '' }) => (
  <div className="fixed inset-0 pointer-events-none z-0 flex justify-center px-4 md:px-8 lg:px-12 max-w-[1600px] mx-auto">
    <div className="w-full grid grid-cols-4 md:grid-cols-8 lg:grid-cols-12 gap-4 md:gap-6 border-x border-gray-300/40 h-full">
      {Array.from({ length: 12 }).map((_, i) => (
        <div key={i} className={`hidden md:block border-r border-gray-300/40 h-full ${i === 0 ? 'border-l' : ''} ${i < 4 ? 'block' : ''}`}></div>
      ))}
    </div>
  </div>
);
...
export const SectionHeadline: React.FC<{ children: React.ReactNode; className?: string }> = ({ children, className = '' }) => (
  <h2 className={`text-5xl md:text-6xl lg:text-7xl font-bold text-black leading-tight tracking-tighter uppercase ${className}`}>
    {children}
  </h2>
);

export const SubHeadline: React.FC<{ children: React.ReactNode; className?: string }> = ({ children, className = '' }) => (
  <h3 className={`text-xl md:text-2xl lg:text-3xl font-normal text-gray-800 leading-snug tracking-tight ${className}`}>
    {children}
  </h3>
);
...
export const MonoLabel: React.FC<{ children: React.ReactNode; className?: string; color?: string }> = ({ children, className = '', color = 'text-gray-500' }) => (
  <div className={`flex items-center gap-2 ${className}`}>
    <div className={`w-1.5 h-1.5 ${color === 'text-blue-swiss' ? 'bg-blue-swiss' : 'bg-black'}`}></div>
...
export const BodyText: React.FC<{ children: React.ReactNode; className?: string }> = ({ children, className = '' }) => (
  <p className={`text-base md:text-lg text-gray-600 leading-relaxed font-normal max-w-2xl ${className}`}>
    {children}
  </p>
);
...
export const AccentLine: React.FC<{ className?: string }> = ({ className = '' }) => (
  <div className={`h-[2px] bg-blue-swiss w-12 mb-8 ${className}`} />
);
...
export const SectionNumber: React.FC<{ number: string; className?: string }> = ({ number, className = '' }) => (
  <div className={`absolute z-0 pointer-events-none select-none overflow-hidden ${className}`}>
    <span className="block text-[20vw] leading-none font-bold text-gray-200 opacity-60 tracking-tighter font-sans -translate-y-[15%]">
      {number}
    </span>
  </div>
);
...
export const ButtonOutline: React.FC<{ 
  ...
  <button
    ...
    className={`
      ...
      bg-transparent text-black font-medium text-sm
      py-5 px-10 tracking-widest uppercase
      border border-gray-300
      transition-all duration-300 ease-out
      hover:border-blue-swiss hover:text-blue-swiss
      ${className}
    `}
  >
    {children}
...
export const TextLink: React.FC<{ 
  ...
  <a 
    ...
    className={`
      ...
      text-black font-medium text-sm tracking-widest uppercase
      ${className}
    `}
  >
    <span className="relative">
      {children}
      <span className="absolute left-0 -bottom-1 w-full h-[1px] bg-gray-300 group-hover:bg-blue-swiss transition-colors duration-300"></span>
    </span>
    <div className="w-8 h-8 flex items-center justify-center rounded-full border border-gray-300 group-hover:border-blue-swiss group-hover:bg-blue-swiss group-hover:text-white transition-all duration-300">
...
export const TextArea: React.FC<React.TextareaHTMLAttributes<HTMLTextAreaElement> & { label: string }> = ({ label, id, className = '', ...props }) => (
  <div className="w-full group">
    ...
    <textarea
      id={id}
      className={`
        w-full min-h-[160px] bg-transparent border-b border-gray-300 
        text-black font-sans text-lg
        focus:border-blue-swiss focus:outline-none
...
export const Card: React.FC<{ children: React.ReactNode; className?: string; noHover?: boolean }> = ({ children, className = '', noHover = false }) => (
  <div className={`
    relative bg-white border border-gray-200 p-8 md:p-12 shadow-sm
    ${!noHover && 'group hover:border-blue-swiss transition-colors duration-300 hover:shadow-md'}
    ${className}
  `}>
    {children}
    {/* Corner accents */}
    ...
    <div className="absolute bottom-0 right-0 w-2 h-2 border-b border-r border-gray-400 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
  </div>
);